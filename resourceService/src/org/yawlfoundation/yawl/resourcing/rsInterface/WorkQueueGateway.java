...
import org.yawlfoundation.yawl.resourcing.SystemQueueSet;
...
public class WorkQueueGateway extends HttpServlet {
    ...
    private String doAction(String action, HttpServletRequest req) throws IOException {
        ...
        //limingyao
        else if (action.equals("getSystemQueues")) {
        	SystemQueueSet sqSet = _rm.getSystemQueues();
        	result = sqSet.toXML();
        }
        //limingyao
        else if (action.equals("getSystemWorkItems")) {
        	SystemQueueSet sqSet = _rm.getSystemQueues();
            if (sqSet != null) {
               Set<WorkItemRecord> set = sqSet.getQueuedWorkItems(WorkQueue.SCHEDULING);
               set.addAll(sqSet.getQueuedWorkItems(WorkQueue.WAITING));
