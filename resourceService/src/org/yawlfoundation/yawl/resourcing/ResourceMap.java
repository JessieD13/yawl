...
public class ResourceMap {
		...
		if (isPersisting()) {
			String where = String
					.format("_specID.identifier='%s' and _specID.version.version='%s' and _taskID='%s'",
							_specID.getIdentifier(),
							_specID.getVersionAsString(), _taskID);
	...
	private void restorePiledResource() {
		...
		if (map != null) {
			if (rm.isPersistPiling()) {
				...
				_piledResource = rm.getOrgDataSet().getParticipant(
						_piledResourceID);
			} else
				_persister.delete(map);
		}
	...
	// limingyao: autoDistributed() used to add the workitems to the correct
	// queue, for the latter allocation.
	public WorkItemRecord autoDistribute(WorkItemRecord wir) {
		boolean routed = false;
		...
		if (!routed) {
			...
			if (distributionSet != null) {
				...
				if (!routed) {
					...
					removeIgnoredParticipants(wir, distributionSet);
					if (!distributionSet.isEmpty()) {
						SystemQueueManager sqM = SystemQueueManager.getInstance();
						rm.getWorkItemCache().updateResourceStatus(wir,
								WorkItemRecord.statusResourceUnoffered);

						sqM.addToScheduling(wir);
	...
	private HashSet<Participant> doOffer(WorkItemRecord wir) {
		...
		if (_offer.getInitiator() == AbstractInteraction.USER_INITIATED) {
			...
			if (offerSet.isEmpty()) {
				_log.warn("Parse of resource specifications for workitem "
						+ wir.getID()
	...
	private Participant doAllocate(HashSet<Participant> pSet, WorkItemRecord wir) {
		...
		if (_allocate.getInitiator() == AbstractInteraction.USER_INITIATED) {
			...
			if (chosenOne == null) {
				_log.warn("The system allocator '"
						+ _allocate.getAllocator().getName()
		...
		if (eleSpec != null) {
			...
			try {
				...
				_privileges.parse(eleSpec.getChild("privileges", nsYawl),
						nsYawl);
				_log.info("Resourcing specification parse completed for task: "
						+ _taskID);
			} catch (ResourceParseException rpe) {
				_log.error("Error parsing resourcing specification for task: "
						+ _taskID, rpe);
			}
		}
	}
