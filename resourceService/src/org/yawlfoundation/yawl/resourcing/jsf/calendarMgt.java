...
import org.yawlfoundation.yawl.resourcing.calendar.ResourceCalendar.RepeatMode;
...
public class calendarMgt extends AbstractPageBean {
    ...
    public void setLblStatus(Label lbl) { lblStatus = lbl; }

    //xuyang lbl RepeatMode
    public Label lblRepeatMode = new Label();

    public Label getLblRepeatMode() { return lblRepeatMode; }

    public void setLblRepeatMode(Label lbl) { lblRepeatMode = lbl; }
    ...
    //xuyang selectStatus
    private DropDown cbbStatus = new DropDown();

    public DropDown getCbbStatus() { return cbbStatus; }

    public void setCbbStatus(DropDown cbb) { cbbStatus = cbb; }
    ...
    //XUYANG
    private DropDown cbbRepeatMode = new DropDown();

    public DropDown getCbbRepeatMode() { return cbbRepeatMode; }

    public void setCbbRepeatMode(DropDown cbb) { cbbRepeatMode = cbb; }
    ...
    //xuyang
    //TODO status valueChangeListener
    public void cbbStatus_processValueChange(ValueChangeEvent event) {
        _sb.setCalStatusSelection((String) event.getNewValue());
    }
    ...
    //xuyang repeapt value change listener
    public void cbbRepeatMode_processValueChange(ValueChangeEvent event) {
        _sb.setCalRepeatModeSelection((String) event.getNewValue());
    }
    /**
    ...
    public String btnAdd_action() {
        if (validateFields()) {
            ...
            // TODO chenyinye: get status from jsp components
            //xuyang
            ResourceCalendar.Status status = getStatus();
            //print test
            ...
            if (startTime < endTime) {
                ...
                if (getMode() == Mode.Edit) {                  // update existing entry
                    ...
                    if (addCalendarEntry(startTime, endTime, status, workload, comment)) {
                        clearFields();
                    }
                }
            }
    ...
    //TODO xuyang select status get items
    public Option[] getCalendarMgtStatusComboItems() {
        Option[] options = new Option[5];
        // options[0] = new Option("available");
        // options[1] = new Option("unavailable");
        options[0] = new Option("workDay");
        options[1] = new Option("extraWork");
        options[2] = new Option("privateHoliday");
        options[3] = new Option("medicalLeaveOfAbsence");
        options[4] = new Option("casualLeaveOfAbsence");
        return options;
    }

  //TODO xuyang select repeat get items
    public Option[] getCalendarMgtRepeatModeComboItems() {
        Option[] options = new Option[3];
        options[0] = new Option("DAY_OF_WEEK");
        options[1] = new Option("DAY_OF_MONTH");
        options[2] = new Option("DAY_OF_YEAR");
        return options;
    }
    /********************************************************************/
    ...
    //xuyang
    private Date incMonth(Date date, int amt) {
        _greg.setTimeInMillis(date.getTime());
        _greg.add(GregorianCalendar.MONTH, amt);
        return _greg.getTime();
        //judge some special date in month
    }

    //xuyang
    private Date incYear(Date date, int amt) {
        _greg.setTimeInMillis(date.getTime());
        _greg.add(GregorianCalendar.YEAR, amt);
        return _greg.getTime();
        //judge some special date in month
    }
    ...
    private void processMode() {
        ...
        //modified by XuYang
        cbbStatus.setDisabled(isEditMode);
        cbbRepeatMode.setDisabled(isEditMode);
    ...
    //TODO****xuyang - 选择repeateMode后的逻辑操作
    /***
    ...
    **/
 // TODO chenyinye: add "status" parameter
 //   TODO****xuyang - 选择repeateMode后的逻辑操作
    private boolean addCalendarEntry(long startTime, long endTime, ResourceCalendar.Status status,
    		int workload, String comment) {
        try {
           /* if (cbxRepeat.isChecked()) {
        	...
        	//4/13update xuyang
            Date baseDate = calComponent.getSelectedDate();
            Date endDate = calDuration.getSelectedDate();
            
            String startTimeStr = (String) txtStartTime.getText();
            String endTimeStr = (String) txtEndTime.getText();

        	switch(RepeatMode.valueOf(getRepeatMode())) {
                case DAY_OF_WEEK:
                    while(startTime < endTime) {
                        _sb.addCalendarEntry(startTime, getTime(endTimeStr, baseDate.getTime()), 
                            status, workload, comment);
                        baseDate = incDate(baseDate, 7);
                        startTime = getTime(startTimeStr, baseDate.getTime());
                    }
                case DAY_OF_MONTH:
                    while(startTime < endTime) {
        	...
        	// _sb.addCalendarEntry(startTime, endTime, status, workload, comment);
    ...
    //xuyang status setter
    private void setStatus() {
        String selectedStatus = _sb.getCalStatusSelection();
        cbbStatus.setSelected(selectedStatus);    
    }

    //xuyang status getter
    private ResourceCalendar.Status getStatus() {
    	// ResourceCalendar.Status status = strToStatus(cbbStatus.getItems().toString());
    	ResourceCalendar.Status status = strToStatus((String) cbbStatus.getSelected());
        return status;
    }
    
    //xuyang 为什么用不了ResourceCalendar里面的
    private ResourceCalendar.Status strToStatus(String name) {
		// TODO Auto-generated method stub
    	return ResourceCalendar.Status.valueOf(name);
	}
    //xuyang repeat mode setter
    private void setRepeatMode() {
        String selectedRepeatMode = _sb.getCalRepeatModeSelection();
        cbbStatus.setSelected(selectedRepeatMode);    
    }
    
    //xuyang repeat mode getter
    private String getRepeatMode(){
    	String result = (String)cbbRepeatMode.getSelected();
    	return result;
    }
