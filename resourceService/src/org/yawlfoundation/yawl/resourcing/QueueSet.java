...
public class QueueSet {
	...
	private int getStartQueue() {
		return (_type == setType.adminSet) ? WorkQueue.UNOFFERED
				: WorkQueue.OFFERED;
	}

	private int getEndQueue() {
		return (_type == setType.adminSet) ? WorkQueue.WORKLISTED
				: WorkQueue.SUSPENDED;
	}

	// limingyao
	/**
	 * Return the number of workitems in the participant queue set
	 * 
	 * @return count
	 */
	public int countWorkItemsInQueues() {
		int count = 0;
		for (int queue = getStartQueue(); queue <= getEndQueue(); queue++) {
			if (!isNullQueue(queue)) {
				count += getQueue(queue).getQueueSize();
			}
		}
		return count;
	}
	 ...
	 */
	public long calTotalExecuteTime() {
		long totalTime = 0;
		for (int queue = WorkQueue.ALLOCATED; queue <= WorkQueue.SUSPENDED; queue++) {
			if (!isNullQueue(queue)) {
				Set<WorkItemRecord> workitems = getQueuedWorkItems(queue);

				for (WorkItemRecord wir : workitems) {
					String exeTime = wir.getExecuteTime();
					if (exeTime != null)
						totalTime += Long.parseLong(exeTime);
	 ...
	 */
	public long calTotalExecuteTime(double delayRate) {
		long totalTime = 0;
		for (int queue = WorkQueue.ALLOCATED; queue <= WorkQueue.SUSPENDED; queue++) {
			if (!isNullQueue(queue)) {
				Set<WorkItemRecord> workitems = getQueuedWorkItems(queue);
				...
				for (WorkItemRecord wir : workitems) {
					String exeTime = wir.getExecuteTime();
					if (exeTime != null && wir.getDelayRate() >= delayRate)
						totalTime += Long.parseLong(exeTime);
	...
	/**
	 * Calculate the time to complete all workitems whose delay rate is not less than given delay rate in the queue set
	 */
	public long calTotalTimeWithDelayRate(double delayRate) {
		long totalTime = 0;
		for (int queue = WorkQueue.ALLOCATED; queue <= WorkQueue.SUSPENDED; queue++)
			if (!isNullQueue(queue)) {
				Set<WorkItemRecord> workitems = getQueuedWorkItems(queue);
				...
				for (WorkItemRecord wir : workitems) {
					String exeTime = wir.getExecuteTime();
					if (exeTime != null && wir.getDelayRate() > delayRate)
						exeTime += Long.parseLong(exeTime);
				}
			}
	...
	/**
	 * Critical workitem means that it was expired before 
	 * or its buffer time equals zero
	 * or it is going to expiry( its delay rate is greater than 90)
	 */
	public boolean hasCriticalWorkItemInAnyQueue() {
		for (int queue = WorkQueue.ALLOCATED; queue <= WorkQueue.SUSPENDED; queue++) {
			if (!isNullQueue(queue)) {
				Set<WorkItemRecord> workitems = getQueuedWorkItems(queue);

				for (WorkItemRecord wir : workitems) {
					if (wir.isCritical() || wir.isEverExpired() || wir.getDelayRate() > 80)
						return true;
				}
			}
		}

		// has no critical workitems
		return false;
	}
