...
public class WorkQueue {
	...
	// limingyao: auto-distributed queue types
	public final static int SCHEDULING = 6;
	public final static int WAITING = 7;
	...
	public WorkQueue(String ownerID, WorkItemRecord item, int qType,
			boolean persisting) {
	...
	public WorkQueue(String ownerID, WorkQueue queue, int qType,
			boolean persisting) {
		this(ownerID, qType, persisting);
		addQueue(queue);
	}
	...
	public void add(WorkItemRecord item, boolean log) {
		_workitems.put(item.getID(), item); // item.getID() = _caseID + ":" +
											// _taskID;
		persistThis();
		if (log)
			logEvent(item);
	...
	public void removeCase(String caseID) {
		Set<WorkItemRecord> clonedQueue = new HashSet<WorkItemRecord>(
				_workitems.values());
	...
	// limingyao
	/**
	 * sort the queue by temporal properties
	 */
	public void sortByDelayTime() {
		// TODO sort the queue
		// Map<String, WorkItemRecord> sortedMap = new ConcurrentHashMap<String,
		// WorkItemRecord>();
		if (_workitems != null && !_workitems.isEmpty()) {
			List<Entry<String, WorkItemRecord>> workitemList = new ArrayList<Entry<String, WorkItemRecord>>(
					_workitems.entrySet());
			Collections.sort(workitemList,
					new Comparator<Entry<String, WorkItemRecord>>() {
						public int compare(Entry<String, WorkItemRecord> obj1,
								Entry<String, WorkItemRecord> obj2) {
							return (obj1.getValue()).compareTo(obj2.getValue());
						}
					});

			Iterator<Entry<String, WorkItemRecord>> iter = workitemList
					.iterator();
			Entry<String, WorkItemRecord> tempEntry = null;
			_workitems.clear();
			while (iter.hasNext()) {
				tempEntry = iter.next();
				_workitems.put(tempEntry.getKey(), tempEntry.getValue());
			}
		}
	}

	/**
	 * sort the queue by the criticality&delay rate
	 */
	public List<WorkItemRecord> getSortedWorkItem() {
		if (_workitems != null && !_workitems.isEmpty()) {
			List<WorkItemRecord> wirs = new LinkedList<WorkItemRecord>(getAll());
			
			//descending sort the workitems
			Collections.sort(wirs,
					new Comparator<WorkItemRecord>() {
				public int compare(WorkItemRecord obj1,
						WorkItemRecord obj2) {
					Boolean isCritical1 = obj1.isCritical();
					Boolean isCritical2 = obj2.isCritical();
					Boolean isExpired1 = obj1.isEverExpired();
					Boolean isExpired2 = obj2.isEverExpired();

					if ((!isCritical1 && isCritical2)
							|| (isCritical1 && !isCritical2))
						return isCritical2.compareTo(isCritical1);
					else if ((!isExpired1 && isExpired2)
							|| (isExpired1 && !isExpired2))
						return isExpired2.compareTo(isExpired1);
					else
		...
		if (element != null) {
			...
			if (items != null) {
				for (Object o : items.getChildren()) {
					WorkItemRecord wir = Marshaller
							.unmarshalWorkItem((Element) o);
