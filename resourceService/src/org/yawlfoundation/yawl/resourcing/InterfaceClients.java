...
public class InterfaceClients {
	...
	private Namespace _yNameSpace = Namespace
			.getNamespace("http://www.yawlfoundation.org/yawlschema");
	...
	public void initClients(String engineURI, String exceptionURI,
		...
		if (engineURI != null) {
			...
			_interfaceEClient = new YLogGatewayClient(engineURI.replaceFirst(
					"/ib", "/logGateway"));
		}
		...
		if (costServiceURI != null) {
			_costServiceClient = new CostClient(costServiceURI,
					_engineLogonName, _engineLogonPassword);
		}
		if (docStoreURI != null) {
			_docStoreClient = new DocStoreClient(docStoreURI, _engineLogonName,
	...
	protected void setServiceURI() {
		...
		if (services != null) {
			for (YAWLServiceReference service : services) {
				// TODO lincaizhao for engine to recognize this service, temp
	...
	protected boolean engineIsAvailable() {
		...
		try {
			available = HttpURLValidator
					.pingUntilAvailable(_engineURI, timeout);
			if (!available) {
	...
	public void announceResourceUnavailable(AbstractResource resource,
		...
		if (resource != null) {
			try {
				String caseData = wir != null ? getCaseData(wir.getRootCaseID())
						: null;
				_gatewayServer.announceResourceUnavailable(resource.getID(),
						wir, caseData, primary);
			} catch (IOException ioe) {
				_log.error(
						"Failed to announce unavailable resource to environment",
						ioe);
			}
		}
	}
	...
	public void announceResourceCalendarStatusChange(String origAgent,
			String changeXML) {
		try {
			_gatewayServer.announceResourceCalendarStatusChange(origAgent,
					changeXML);
		} catch (IOException ioe) {
			_log.error(
					"Failed to announce resource calendar status change to environment",
					ioe);
		}
	}
	...
	public String registerCalendarStatusChangeListener(String uri, String userID) {
		return (userID == null) ? fail("Invalid session handle")
				: _gatewayServer.registerSchedulingInterfaceListener(userID,
						uri);
	}
					...
					if (_interfaceBClient.successful(result)) { // and is online
						try {
							if (_gatewayServer != null) {
								_gatewayServer.redirectWorkItemToYawlService(
										wir.toXML(), serviceURI);
								return SUCCESS_STR;
							} else
								result = fail("Gateway server unavailable");
						} catch (Exception e) {
							_log.error(
									"Failed to redirect workitem: "
											+ wir.getID(), e);
							result = fail(e.getMessage());
						}
					}
				} else
					result = fail("Unknown or unregistered service name: "
							+ serviceName);
			} else
				result = fail("Only work items with enabled or fired status may be "
						+ "redirected; work item ["
						+ wir.getID()
						+ "] has status: " + wir.getStatus());
		} else
	...
	protected String getEngineBuildProperties() {
		try {
			return _interfaceAClient
					.getBuildProperties(getEngineSessionHandle());
		} catch (IOException ioe) {
	...
	public Set<YAWLServiceReference> getRegisteredServices() {
		return _interfaceAClient
				.getRegisteredYAWLServices(getEngineSessionHandle());
	}

	protected String getAdminUserPassword() {
		try {
			return _interfaceAClient.getPassword(ADMIN_STR,
	...
	protected String unloadSpecification(YSpecificationID specID)
			throws IOException {
		return _interfaceAClient.unloadSpecification(specID,
				getEngineSessionHandle());
	}

	public String getRegisteredServicesAsXML() throws IOException {
		return _interfaceAClient
				.getRegisteredYAWLServicesAsXML(getEngineSessionHandle());
	}

	protected String addRegisteredService(YAWLServiceReference service)
			throws IOException {
		return _interfaceAClient.addYAWLService(service,
				getEngineSessionHandle());
	}

	protected String removeRegisteredService(String id) throws IOException {
		return _interfaceAClient
				.removeYAWLService(id, getEngineSessionHandle());
	}
	...
	protected String addExternalClient(YExternalClient client)
			throws IOException {
		return _interfaceAClient.addClientAccount(client,
				getEngineSessionHandle());
	}

	protected String removeExternalClient(String id) throws IOException {
		return _interfaceAClient.removeClientAccount(id,
				getEngineSessionHandle());
	}

	protected String updateExternalClient(String id, String password,
			String doco) throws IOException {
	...
	protected List<WorkItemRecord> getWorkItemsForService() throws IOException {
		return _interfaceBClient.getWorkItemsForService(getServiceURI(),
				getEngineSessionHandle());
	}

	protected String suspendWorkItem(String itemID) throws IOException {
		return _interfaceBClient.suspendWorkItem(itemID,
				getEngineSessionHandle());
	}

	protected String unsuspendWorkItem(String itemID) throws IOException {
		return _interfaceBClient.unsuspendWorkItem(itemID,
				getEngineSessionHandle());
	}
	...
	protected String getSpecificationDataSchema(YSpecificationID specID)
			throws IOException {
		return _interfaceBClient.getSpecificationDataSchema(specID,
				getEngineSessionHandle());
	}

	protected List<WorkItemRecord> getLiveWorkItemsForCase(String caseID) {
		try {
			return _interfaceBClient.getLiveWorkItemsForIdentifier("case",
					caseID, getEngineSessionHandle());
		} catch (Exception e) {
	...
	protected String launchCase(YSpecificationID specID, String caseData,
		...
		return _interfaceBClient.launchCase(specID, caseData,
				getEngineSessionHandle(), logList, _serviceURI);
	}

	// limingyao : Overriding launchCase() with adding case emergency level
	// param
	protected String launchCase(YSpecificationID specID, String caseData,
			YLogDataItemList logList, int emergencyLevel) throws IOException {
		if (_serviceURI == null)
			setServiceURI();
		return _interfaceBClient.launchCase(specID, caseData,
				getEngineSessionHandle(), logList, _serviceURI, emergencyLevel);
	}
	...
	protected String launchCase(YSpecificationID specID, String caseData,
		...
		if (_serviceURI == null)
			setServiceURI();
		return _interfaceBClient.launchCase(specID, caseData,
				getEngineSessionHandle(), logList, _serviceURI, delay);
	}
	...
	public XNode getAllRunningCases() {
		try {
			String caseStr = _interfaceBClient
					.getAllRunningCases(getEngineSessionHandle());
	...
	protected List<String> getRunningCasesAsList(YSpecificationID specID) {
		try {
			String casesAsXML = _interfaceBClient.getCases(specID,
					getEngineSessionHandle());
	...
	protected String getTaskParamsAsXML(YSpecificationID specID, String taskID)
			throws IOException {
		String xml = _interfaceBClient.getTaskInformationStr(specID, taskID,
				getEngineSessionHandle());
		if (xml != null) {
	...
	protected boolean canAddNewInstance(WorkItemRecord wir) {
		try {
			return _interfaceBClient.successful(_interfaceBClient
					.checkPermissionToAddInstances(wir.getID(),
							getEngineSessionHandle()));
		} catch (IOException ioe) {
			return false;
		}
	...
	protected WorkItemRecord createNewWorkItemInstance(String id, String value) {
		...
		try {
			String xml = _interfaceBClient.createNewInstance(id, value,
					getEngineSessionHandle());
	...
	public String getEngineSpecificationStatistics(YSpecificationID specID,
			long from, long to) {
		try {
			return _interfaceEClient.getSpecificationStatistics(specID, from,
					to, getEngineSessionHandle());
		} catch (IOException ioe) {
			return null;
		}
	}
	...
	protected void removeCaseFromDocStore(String caseID) {
		try {
			String response = _docStoreClient.completeCase(caseID,
					_docStoreClient.getHandle());
			_log.debug(response);
		} catch (IOException ioe) {
