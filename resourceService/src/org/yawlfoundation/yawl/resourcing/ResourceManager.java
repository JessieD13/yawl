...
public class ResourceManager extends InterfaceBWebsideController {
	...
	// limingyao: store of system distributed queues
	private SystemQueueManager _sqManager = SystemQueueManager.getInstance();

	// String literals
	public static final String ADMIN_STR = "admin";
	// limingyao :system queue's own id
	...
	public void initOrgDataSource(String dataSourceClassName, int refreshRate) {
		...
		if (_orgdb != null) {
			...
			_isNonDefaultOrgDB = !(_orgdb.getClass().getSimpleName()
					.equalsIgnoreCase("HibernateImpl"));
	...
	// limingyao: Init the task scheduler thread
	public void initTaskDispatcher() {
		_log.info("Starting Task Dispatcher...");

		//
		TaskDispatcher td = TaskDispatcher.getInstance();
		if (td != null)
			td.dispatch();
		else
			_log.warn("Cannot start the task scheduler. ");
	}
	...
	public void removeCalendarStatusChangeListener(String uri, String handle) {
		_services.removeCalendarStatusChangeListener(uri,
				getUserIDForSessionHandle(handle));
	}

	public void removeCalendarStatusChangeListeners(String handle) {
		_services
				.removeCalendarStatusChangeListeners(getUserIDForSessionHandle(handle));
	}
			...
			if (_serviceEnabled) {
				if (_workItemCache.contains(wir)) {
					_log.warn("Duplicate post received for new work item ["
							+ wir.getID() + "] - no further action required.");
					return;
				}
				if (wir.isAutoTask()) {
					...
					// limingyao: handle when it's auto-distributed workitem
					if (rMap != null) {
						if (rMap.getOfferInteraction().getInitiator() == AbstractInteraction.AUTO_INITIATED)
							rMap.autoDistribute(wir);
						else
							rMap.distribute(wir);
					} else
						offerToAll(wir);
					// wir = (rMap != null) ? rMap.distribute(wir) :
					// offerToAll(wir);
				}
			}
	...
	public void handleWorkItemStatusChangeEvent(WorkItemRecord wir,
		...
		synchronized (_ibEventMutex) {
			...
			if (!((cachedWir == null) || newStatus
					.equals(cachedWir.getStatus()))) {
				...
				if ((newStatus.equals(WorkItemRecord.statusComplete))
						...
						|| (newStatus
								.equals(WorkItemRecord.statusForcedComplete))) {
				...
				// move it
					...
					Participant p = getParticipantAssignedWorkItem(cachedWir,
							WorkQueue.STARTED);
					if (p != null) {
						p.getWorkQueues().movetoSuspend(cachedWir);
						cachedWir
								.setResourceStatus(WorkItemRecord.statusResourceSuspended);
					}
				...
				// if it has moved to started status
					...
					if (cachedWir.hasStatus(WorkItemRecord.statusSuspended)) {
						Participant p = getParticipantAssignedWorkItem(
								cachedWir, WorkQueue.SUSPENDED);
						if (p != null) {
							p.getWorkQueues().movetoUnsuspend(cachedWir);
							cachedWir
									.setResourceStatus(WorkItemRecord.statusResourceStarted);
							_workItemCache.updateStatus(cachedWir, newStatus);
						}
					}
				}
	...
	public void handleStartCaseEvent(YSpecificationID specID, String caseID,
		...
		if (delayed && launchingService != null
				&& launchingService.equals(_services.getServiceURI())) {
	...
	// lincaizhao handle temporal timer expired events
	@Override
	public void handleLFTimerExpiryEvent(WorkItemRecord wir) {
		// super.handleLFTimerExpiryEvent();
		_log.debug("handle latest finish timer expiry event" + wir.getID());
		// lincaizhao: simply log the latest finish timer expired event,
		// for the engine will complete the workitem as failed and will
		// announce to the resource service that the workitem is canceled
		EventLogger.log(wir, null, EventLogger.event.lf_timer_expired);
		// if clean up workitem here, the workitem cancel announcement
		// can not find the workitem reference, and since the latest
		// timer expired event cause the cancel announcement, the order
		// of these two event depend on timing, thus we simply log the
		// timer expired event and left the clean up to the cancel event
		/*
		 * if (cleanupWorkItemReferences(wir)) { // remove from worklists
		 * EventLogger.log(wir, null, EventLogger.event.lf_timer_expired); }
		 */
	...
	@Override
	public void handleLSTimerExpiryEvent(WorkItemRecord wir) {
		// super.handleLSTimerExpiryEvent();
		_log.debug("handle latest start timer expiry event" + wir.getID());
		EventLogger.log(wir, null, EventLogger.event.ls_timer_expired);
		if (wir.isAutoTask()) {
			handleAutoTask(wir, true);
		} else {
	...
	public String getCodeletsAsXML() {
		...
		for (AbstractCodelet codelet : codelets)
	...
	public void loadResources() {
		if (_orgdb != null) {
			...
			_resAdmin.createWorkQueues(_persisting); // ... and the
														// administrator

			// TODO limingyao: create system queues
			_sqManager.createWorkQueues(_persisting);
	...
	private List<String> syncCacheWithEngineItems() {
		...
		try {
			List<WorkItemRecord> engineItems = _services
	...
	private void removeOrphanedItemsFromCache(List<String> engineIDs) {
		...
		for (String missingID : missingIDs) {
			...
			try {
				...
				// err on the side of caution and keep item in cache - ie. do
				// nothing
			}
		}
	}
	...
	private void finaliseNonDefaultLoad() {
		...
		for (Participant p : _orgDataSet.getParticipants()) {
			...
			if (up != null)
				...
				p.setUserPrivileges(new UserPrivileges(p.getID()));
		}
	}
	...
	private void restoreWorkQueues() {
		...
		if (qList != null) {
			for (WorkQueue wq : qList) {
				...
				// TODO limingyao: if it's a system queue, it has no owner id
				else if (wq.getOwnerID().equals(SYSTEM_STR)) {
					_sqManager.getWorkQueues().setQueue(wq);
				} else {
					if (_orgDataSet != null) {
						Participant p = _orgDataSet.getParticipant(wq
								.getOwnerID());
						if (p != null) {
	...
	public Set<Participant> getParticipantsAssignedWorkItem(String workItemID,
		...
		for (Participant p : _orgDataSet.getParticipants()) {
			QueueSet qSet = p.getWorkQueues();
			if ((qSet != null)
					&& (qSet.hasWorkItemInQueue(workItemID, queueType)))
				result.add(p);
		}
		if (result.isEmpty())
			result = null;
	...
	public Participant getParticipantAssignedWorkItem(WorkItemRecord wir,
		...
		if ((qType > WorkQueue.OFFERED) && (qType <= WorkQueue.SUSPENDED)) {
			for (Participant p : _orgDataSet.getParticipants()) {
				QueueSet qSet = p.getWorkQueues();
				if ((qSet != null)
						&& (qSet.hasWorkItemInQueue(wir.getID(), qType))) {
					result = p;
					break;
				}
			}
	...
	public WorkItemRecord offerToAll(WorkItemRecord wir) {
		if (_orgDataSet.getParticipantCount() > 0) {
			_workItemCache.updateResourceStatus(wir,
	...
	public void removeFromAll(WorkItemRecord wir) {
		...
		// limingyao: remove from system queues
		_sqManager.removeFromAllQueues(wir);
	}
	...
	public void removeCaseFromAllQueues(String caseID) {
		...
		// limingyao: remove from system queues
		_sqManager.removeCaseFromAllQueues(caseID);
	}
	...
	//limingyao: get system queues
	public SystemQueueSet getSystemQueues() {
		return _sqManager.getWorkQueues();
	}
	...
	public WorkItemRecord acceptOffer(Participant p, WorkItemRecord wir) {
		...
		if ((starter != null)
			...
			WorkItemRecord startedItem = getExecutingChild(getChildren(wir
					.getID()));
			if (startedItem != null) {
				WorkItemRecord cachedItem = _workItemCache.get(startedItem
						.getID());
				if (cachedItem != null)
					wir = cachedItem;
			}
	...
	private void withdrawDeferredChoiceGroup(WorkItemRecord wir,
			ResourceMap rMap) {
			...
			if ((qOffer != null) && (!qOffer.isEmpty())) {
				...
				for (Participant temp : _orgDataSet.getParticipants()) {
					if ((p == null) || (!temp.getID().equals(p.getID()))) {
						WorkQueue q = temp.getWorkQueues().getQueue(
								WorkQueue.OFFERED);
						if (q != null)
				...
				for (WorkItemRecord wir : wirSet) {
					if (!offerSet.contains(wir)) {
						_resAdmin.getWorkQueues().removeFromQueue(wir,
	...
	public void handleAllocatedQueueOnRemoval(WorkQueue qAlloc) {
		synchronized (_removalMutex) {
			...
			if ((qAlloc != null) && (!qAlloc.isEmpty())) {
				_resAdmin.getWorkQueues().removeFromQueue(qAlloc,
						WorkQueue.WORKLISTED);
				_resAdmin.getWorkQueues().addToQueue(WorkQueue.UNOFFERED,
						qAlloc);
	...
	public boolean start(Participant p, WorkItemRecord wir) {
		...
		if (checkOutWorkItem(wir)) {
			...
			if (wir.getResourceStatus().equals(
					WorkItemRecord.statusResourceUnoffered)) {
				_resAdmin.getWorkQueues().removeFromQueue(wir,
						WorkQueue.UNOFFERED);
			}
	...
	private WorkItemRecord getStartedChild(WorkItemRecord wir) {
		...
		if (children == null) {
			_log.error("Checkout of workitem '" + wir.getID()
					+ "' unsuccessful.");
			return null;
		}
	...
	private void distributeChildren(WorkItemRecord started,
	...
	private boolean secondaryResourcesAvailable(WorkItemRecord wir,
			Participant p) {
		ResourceMap map = getResourceMap(wir);
		if (!((map == null) || map.getSecondaryResources().available(wir))) {
			_log.warn("Workitem '"
					+ wir.getID()
					+ "' could not be started due "
	...
	public boolean suspendWorkItem(Participant p, WorkItemRecord wir) {
		...
		if (hasUserTaskPrivilege(p, wir, TaskPrivileges.CAN_SUSPEND)) {
			try {
				...
				_log.error(
						"Exception trying to suspend work item: " + wir.getID(),
						ioe);
			}
		}
		return success;
	...
	public boolean unsuspendWorkItem(Participant p, WorkItemRecord wir) {
		...
		try {
			...
			_log.error(
					"Exception trying to unsuspend work item: " + wir.getID(),
					ioe);
		}
		return success;
	}
	...
	public boolean reallocateStatelessWorkItem(Participant pFrom,
			Participant pTo, WorkItemRecord wir) {
		boolean success = false;
		if (hasUserTaskPrivilege(pFrom, wir,
			...
			reallocateWorkItem(pFrom, pTo, wir,
					EventLogger.event.reallocate_stateless);
			success = true;
		}
		return success;
	}
	...
	public boolean reallocateStatefulWorkItem(Participant pFrom,
		...
		if (hasUserTaskPrivilege(pFrom, wir,
			...
			reallocateWorkItem(pFrom, pTo, wir,
					EventLogger.event.reallocate_stateful);
			success = true;
		}
		return success;
	}
	...
	private boolean routeWorkItem(Participant p, WorkItemRecord wir,
			String handle) {
		if (handle != null)
			start(p, wir);
		return (handle != null);
	...
	public boolean hasUserTaskPrivilege(Participant p, WorkItemRecord wir,
		...
		return (taskPrivileges != null)
				&& taskPrivileges.hasPrivilege(p, privilege);
	}
		...
		if ((data != null) && (data.length() > 0)) {
			...
			if (wir != null) {
				if (wir.getStatus().equals(WorkItemRecord.statusExecuting)) {
					...
					if (dataElem != null) {
						String validate = checkWorkItemDataAgainstSchema(wir,
								dataElem);
	...
	private String checkWorkItemDataAgainstSchema(WorkItemRecord wir,
			Element data) {
		...
		if (!data.getName().equals(wir.getTaskName().replace(' ', '_'))) {
			...
			try {
				...
				if (validator.validateSchema()) {
					...
					// fails
					validator.validate(taskInfo.getParamSchema()
							.getCombinedParams(), data, "");
				} else
	...
	private void removeActiveCalendarEntriesForCase(String caseID) {
		try {
			...
			_log.error("Could not clear Calendar bookings for case: " + caseID,
					ce);
	...
	public ResourceMap getPersistedPiledTask(YSpecificationID specID,
		...
		String where = String
				.format("_specID.identifier='%s' and _specID.version.version='%s' and _taskID='%s'",
	...
	public boolean mapSetContains(Set<ResourceMap> mapSet, ResourceMap other) {
		for (ResourceMap map : mapSet) {
			if (map.equals(other)
					&& map.getPiledResourceID().equals(
							other.getPiledResourceID())) {
				return true;
			}
		}
		return false;
		...
		try {
			...
			if (wir != null) {
				...
				result = checkInWorkItem(wir.getID(), wir.getDataList(),
						outData, getCompletionLogPredicate(p, wir),
						getEngineSessionHandle());
	...
	private String getCompletionLogPredicate(Participant p, WorkItemRecord wir) {
		...
		if (wir.isDocumentationChanged()) {
			YLogDataItem docoItem = new YLogDataItem("Predicate",
					"Documentation", wir.getDocumentation(), "string");
			YLogDataItemList itemList = new YLogDataItemList(docoItem);
			if (decompPredicate != null) {
				YLogDataItem decompItem = new YLogDataItem("Predicate",
						"Complete", decompPredicate, "string");
	...
	private String parseCompletionLogPredicate(Participant p, WorkItemRecord wir) {
		...
		return (predicate != null) ? new LogPredicateParser(p, wir)
				.parse(predicate) : null;
	}
	...
	public boolean isSpecBetaVersion(WorkItemRecord wir) {
		...
		return (specData != null)
				&& specData.getSchemaVersion().isBetaVersion();
	}
	 ...
	 */
	public String cancelCase(String caseID, String userHandle)
			throws IOException {
		synchronized (_ibEventMutex) {
			List<WorkItemRecord> liveItems = _services
					.getLiveWorkItemsForCase(caseID);
			...
			if (successful(result)) {
				if (liveItems != null) {
					for (WorkItemRecord wir : liveItems) {
						...
						freeSecondaryResources(wir);
						_workItemCache.remove(wir);
						EventLogger.log(wir, null,
								EventLogger.event.cancelled_by_case);
					}
					_cache.removeChainedCase(caseID);
				}

				// log the cancellation
				Participant p = _cache
						.getParticipantWithSessionHandle(userHandle);
	...
	public String launchCase(YSpecificationID specID, String caseData,
		...
		String caseID = _services.launchCase(specID, caseData,
				getLaunchLogData());
		if (successful(caseID)) {
			EventLogger.log(specID, caseID, _cache.getWhoLaunchedCase(handle),
					true);
		}
		return caseID;
	}

	// limingyao: Overload launchCase() with adding emergency level parameter
	public String launchCase(YSpecificationID specID, String caseData,
			String handle, int emergencyLevel) throws IOException {
		String caseID = _services.launchCase(specID, caseData,
				getLaunchLogData(), emergencyLevel);
		if (successful(caseID)) {
			EventLogger.log(specID, caseID, _cache.getWhoLaunchedCase(handle),
					true);
		}
	...
	public String launchCase(YSpecificationID specID, String caseData,
			String handle, long delay) throws IOException {
		String result = _services.launchCase(specID, caseData,
				getLaunchLogData(), delay);
		if (successful(result)) {
			_cache.addDelayedCaseLaunch(new DelayedLaunchRecord(specID, _cache
	...
	public String launchCase(YSpecificationID specID, String caseData,
			String handle, Date delay) throws IOException {
		String result = _services.launchCase(specID, caseData,
				getLaunchLogData(), delay);
		if (successful(result)) {
			_cache.addDelayedCaseLaunch(new DelayedLaunchRecord(specID, _cache
	...
	public String launchCase(YSpecificationID specID, String caseData,
			String handle, Duration delay) throws IOException {
		String result = _services.launchCase(specID, caseData,
				getLaunchLogData(), delay);
		if (successful(result)) {
			_cache.addDelayedCaseLaunch(new DelayedLaunchRecord(specID, _cache
	...
	private YLogDataItemList getLaunchLogData() {
		return new YLogDataItemList(new YLogDataItem("launched", "name",
	...
	public String getTaskParamsAsXML(String itemID) throws IOException {
		WorkItemRecord wir = _workItemCache.get(itemID);
		if (wir != null) {
			return _services.getTaskParamsAsXML(new YSpecificationID(wir),
					wir.getTaskID());
		} else
	...
	public String getOutputOnlyTaskParamsAsXML(String itemID) {
		...
		if (wir != null) {
			try {
				...
				List<YParameter> list = taskInfo.getParamSchema()
						.getOutputOnlyParams();
		...
		for (String name : outputs.keySet()) {
			...
			if (data != null) {
				if (data.getContentSize() > 0) // complex type
					outputs.get(name).setValue(
							JDOMUtil.elementToStringDump(data));
				else
					// simple type
	...
	public String addRegisteredService(YAWLServiceReference service)
		...
		if (successful(result)) {
			_cache.addClientCredentials(service.getServiceName(),
					service.getServicePassword());
		}
		return result;
	}
	...
	public String updateExternalClient(String id, String password, String doco)
		...
		if (successful(result)) {
			_cache.updateClientCredentials(id,
	...
	private void setAuthorisedServiceConnections() {
		...
		try {
			...
			Set<YAWLServiceReference> services = _services
					.getRegisteredServices();
			if (services != null) {
				for (YAWLServiceReference service : services) {
					users.put(service.getServiceName(),
							service.getServicePassword());
				}
			}
	...
	public String getNetParamValue(String caseID, String paramName)
			throws IOException {
		String result = null;
	...
	public Map<String, Element> getSpecificationDataSchema(
			YSpecificationID specID) throws IOException {
	...
	public String getInstanceData(String schema, YSpecificationID specID) {
		...
		SpecificationData specData = getSpecData(specID);
		if (specData != null) {
			result = new DataSchemaProcessor().getInstanceData(schema,
					specData.getRootNetID(), null);
		}
		return result;
	}
	...
	public String getInstanceData(String schema, WorkItemRecord wir) {
		...
		try {
			...
			if (taskInfo != null)
				result = new DataSchemaProcessor().getInstanceData(schema,
						taskInfo.getDecompositionID(), wir.getDataListString());
		} catch (IOException ioe) {
			result = null;
		}
		return result;
		...
		if (wir != null) {
			if (action.equals("Start")) {
				...
				if (status.equals(WorkItemRecord.statusEnabled)
					...
					_log.error("Unable to start workitem due to invalid status: "
							+ status);
					result = false;
				}
	...
	private void handleAutoTask(WorkItemRecord wir, boolean timedOut) {
		...
		synchronized (_autoTaskMutex) {
			...
			if (checkOutWorkItem(wir)) {
				...
				_log.error("Could not check out automated workitem: "
						+ wir.getID());
		}
	}
	...
	private void processAutoTask(WorkItemRecord wir, boolean init) {
		try {
			...
			if (!StringUtil.isNullOrEmpty(codelet)) {
				TaskInformation taskInfo = getTaskInformation(wir);
				if (taskInfo != null) {
					CodeletRunner runner = new CodeletRunner(wir, taskInfo,
							init);
					Thread runnerThread = new Thread(runner, wir.getID()
							+ ":codelet");
					...
					_log.error(MessageFormat
							.format("Could not run codelet ''{0}'' for workitem ''{1}'' - error "
			...
			_log.error(
					"Exception attempting to execute automatic task: "
							+ wir.getID(), e);
		}
	}
	...
	private void persistAutoTask(WorkItemRecord wir, boolean isSaving) {
		if (isSaving) {
			...
			PersistedAutoTask task = (PersistedAutoTask) _persister
					.selectScalar("PersistedAutoTask", wir.getID());
			if (task != null)
				task.unpersist();
		}
		_persister.commit();
	...
	public void handleCodeletCompletion(WorkItemRecord wir,
		...
		if (_cache.removeCodeletRunner(wir.getID()) != null) {
			if (codeletResult != null) {
				codeletResult = updateOutputDataList(wir.getDataList(),
						codeletResult);
			}
			Element outData = (codeletResult != null) ? codeletResult : wir
					.getDataList();
			checkInAutoTask(wir, outData);
		} else {
			_log.warn("A codelet has completed for a non-existent workitem '"
					+ wir.getID()
	...
	public void checkInAutoTask(WorkItemRecord wir, Element outData) {
		...
		try {
			...
			if (successful(msg)) {
				...
				_log.info("Automated task '"
						+ wir.getID()
						+ "' successfully processed and checked back into the engine.");
			} else
				_log.error("Automated task '"
						+ wir.getID()
						+ "' could not be successfully completed. Result message: "
						+ msg);
		} catch (Exception e) {
	...
	public String getMIFormalInputParamName(WorkItemRecord wir) {
		...
		if (canAddNewInstance(wir)) {
			try {
				TaskInformation taskInfo = getTaskInformation(wir);
				YParameter formalInputParam = taskInfo.getParamSchema()
						.getFormalInputParam();
	...
	private TaskInformation getTaskInformation(WorkItemRecord wir)
			throws IOException {
		return getTaskInformation(new YSpecificationID(wir), wir.getTaskID());
	}

	private TaskInformation getTaskInformation(YSpecificationID specID,
	...
	public String redirectWorkItemToYawlService(String itemID,
		...
		String result = _services.redirectWorkItemToYawlService(wir,
				serviceName);
		if (successful(result)) {
			removeFromAll(wir);
