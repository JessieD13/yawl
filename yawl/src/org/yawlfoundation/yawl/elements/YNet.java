...
public final class YNet extends YDecomposition {
    ...
    // TODO update to deal with the iteration version
    /* lincaizhao: initialize case valid time and calculate EFT(Earliest Finish
     * Time) and LFT(Latest Finish Time) for each task
     * note: this method assume that there is <strong>NO</strong> iteration
     * structure in the net, otherwise, it may cause dead loop
     */
    public void initTemporalStates(TimeSpan caseValidTime) {
    	Queue<YExternalNetElement> queue = new LinkedList<YExternalNetElement>();
    	
    	TimePoint now = new TimePoint();
    	for (YExternalNetElement elem : _inputCondition.getPostsetElements()) {
    		// it should be a task
    		if (elem instanceof YTask) {
    			YTask task = (YTask)elem;
    			task.setRegularStartTime(now);
    			queue.offer(elem);
    		}
    	}
    			...
    			if (preCond instanceof YCondition) {
    				for (YExternalNetElement pre : preCond.getPresetElements()) {
    					if (pre instanceof YTask) {
    						...
    						preRst = preRst.forward(preTask.getMaxExecuteTime());
    	...
    	TimePoint end = now.forward(caseValidTime);
    	for (YExternalNetElement elem : _outputCondition.getPresetElements()) {
    		// it should be a task
    		if (elem instanceof YTask) {
    			YTask task = (YTask)elem;
    			task.setLatestStartTime(end.backward(task.getMaxExecuteTime()));
    			queue.offer(elem);
    		}
    	}
    	// calculate LST(Latest Start Time)
    			...
    			if (postCond instanceof YCondition) {
    				for (YExternalNetElement post : postCond.getPostsetElements()) {
    					if (post instanceof YTask) {
    						...
    						postLst = postLst.backward(cur.getMaxExecuteTime());
