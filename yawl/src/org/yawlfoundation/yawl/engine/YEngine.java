...
public class YEngine implements InterfaceADesign,
   ...
   /*****************************************************************************/
    ...
    public void addRunner(YNetRunner runner, YSpecification specification) {
        if (specification != null) {
            ...
            _instanceCache.addCase(runner.getCaseID().toString(),
                                  ...
                                  runner.getStartTime(), runner.getValidTime());
    ...
    //limingyao : Overload startCase() with emergency level param
    protected YIdentifier startCase(YSpecificationID specID, String caseParams,
          	  						URI completionObserver, String caseID,YLogDataItemList logData, 
          	  						int emergencyLevel, String serviceRef, boolean delayed)
    	...
    	if (specification != null) {
    		...
    		// lincaizhao: check specification valid time interval
    		TimePoint now = new TimePoint();
    		if (!now.during(specification.getSpecValidTime())) {
    			_logger.debug("specification with ID [" + specID + "] start failed, is not valid now");
    			throw new YStateException("specification with ID [" + specID + "] is not valid now");
    		}

    		// check & format case data params (if any)
    		Element data = formatCaseParams(caseParams, specification);

    		//YNetRunner runner = new YNetRunner(_pmgr, specification.getRootNet(), data, caseID, specification.getCaseValidTime());
    		//limingyao: modify the case valid timespan according to the emergencyLevel
    		YNetRunner runner;
    		
    		//Read the emergency level parameters from property file
    		InputStream in = getClass().getResourceAsStream("caseValidTimeParams.properties");
    		...
    		TimeSpan defaultValidTime = specification.getCaseValidTime();
    		double normal = Double.parseDouble(p.getProperty("normal"));
    		double important = Double.parseDouble(p.getProperty("important"));
    		double urgent = Double.parseDouble(p.getProperty("urgent"));
    		
    		if( emergencyLevel == 1){
    			runner = new YNetRunner(_pmgr, specification.getRootNet(), data, caseID, defaultValidTime.multiply(normal), normal);
    		}
    		else if( emergencyLevel == 2){
    			runner = new YNetRunner(_pmgr, specification.getRootNet(), data, caseID, defaultValidTime.multiply(important), important);
    		}
    		else if ( emergencyLevel == 3){
    			runner = new YNetRunner(_pmgr, specification.getRootNet(), data, caseID, defaultValidTime.multiply(urgent), urgent);
    		}
    		else{
    			runner = new YNetRunner(_pmgr, specification.getRootNet(), data, caseID, defaultValidTime, normal);
    		}
    		//////////////////////////////////
    ...
    private void logCaseStarted(YSpecificationID specID, YNetRunner runner,
        ...
        _instanceCache.addCase(caseID.toString(), specID, caseParams,
                logData, runner.getStartTime(), runner.getValidTime());
    }
    ...
    //limingyao: Overload launchCase() with emergency level
    public String launchCase(YSpecificationID specID, String caseParams,
            				URI completionObserver, YLogDataItemList logData,
            				int emergencyLevel, String serviceHandle)
            throws YStateException, YDataStateException,
            YPersistenceException, YEngineStateException, YQueryException {
    	return launchCase(specID, caseParams, completionObserver, null, logData, 
    			emergencyLevel, serviceHandle, false);
    }
    
    public String launchCase(YSpecificationID specID, String caseParams,
            				URI completionObserver, String caseID, YLogDataItemList logData, 
            				int emergencyLevel, String serviceHandle, boolean delayed)
    	...
    	synchronized(_pmgr) {
    		...
    		try {
    			YIdentifier yCaseID = startCase(specID, caseParams, completionObserver,
    					caseID, logData, emergencyLevel, serviceHandle, delayed);
    			if (yCaseID != null) {
    				commitTransaction();
        ...
        if (childCaseIDs != null) {
            ...
            for (YIdentifier childID : childCaseIDs) {
                ...
                if (! oneStarted) {
                    ...
                    oneStarted = true;
                    // TODO lincaizhao: remove latest start timer
                    // TODO lincaizhao: for multiple instance task, when to 
                    // remove the latest start timer? now or when the post
                    // condition is actived?
                    if (workItem.getLatestStartTime() != null) {
    ...
    private void cleanupCompletedWorkItem(YWorkItem workItem, YNetRunner netRunner,
        ...
        // TODO remove latest finish timer
        if (workItem.getLatestStartTime() != null) {
        	YTimer.getInstance().cancelTimerTask(workItem.getIDString());
        }
