...
import org.yawlfoundation.yawl.engine.time.YLatestStartTimer;
import org.yawlfoundation.yawl.engine.time.YLatestFinishTimer;
...
public class YWorkItem {
    ...
    // lincaizhao: temporal properties
    private TimePoint _regularStartTime;
    private TimePoint _latestStartTime;
    private TimeSpan _executeTime;
    
    // limingyao: expiry flag
    private boolean _isEverExpired;
    ...
    public YWorkItem createChild(YPersistenceManager pmgr, YIdentifier childCaseID)
        ...
        if (this._parent == null) {
            ...
            // lincaizhao: set temporal properties
            childItem.setLatestStartTime(getLatestStartTime());
            childItem.setRegularStartTime(getRegularStartTime());
            childItem.setExecuteTime(getExecuteTime());

            // limingyao: set expiry flag
            childItem.setIsEverExpired(getIsEverExpired());
 ...
 // lincaizhao
    /** check and start latest start timer, should only called when first enabled */
    public void checkLatestStartTimer(YPersistenceManager pmgr) 
    		throws YPersistenceException {
    	YLatestStartTimer lstimer = null;
    	if (_latestStartTime != null) {
    		lstimer = new YLatestStartTimer(_workItemID.toString(),
    				_latestStartTime.getMillisecond(), pmgr != null);
    		if (lstimer != null) {
    			if (pmgr != null) pmgr.storeObject(lstimer);
    		}
    	}
    }
    
    /** check and start latest finish timer, should only called when start item */
    public void checkLatestFinishTimer(YPersistenceManager pmgr)
    		throws YPersistenceException {
    	YLatestFinishTimer lftimer = null;
    	if (_latestStartTime != null) {
    		TimePoint lft = _latestStartTime.forward(_executeTime);
    		lftimer = new YLatestFinishTimer(_workItemID.toString(),
    				lft.getMillisecond(), pmgr != null);
    		if (lftimer != null) {
    			if (pmgr != null) pmgr.storeObject(lftimer);
    		}
    	}
    }
    ...
    //limingyao
    /**
     * Query the expiry flag for workitem when enabled from logs
     */
    public boolean getIsEverExpiredFromLog(YPersistenceManager pmgr){
    	String specID = _specID.getIdentifier();
    	String taskID = _task.getID();
    	String quotedID = String.format("'%s:%s'", specID, taskID);
    	YExpiryTask expirytask = new YExpiryTask();
		try {
			expirytask = (YExpiryTask)pmgr.selectScalar("YExpiryTask", "itemID", quotedID);
    ...
    public void setStatusToStarted(YPersistenceManager pmgr, YClient client)
        ...
        // TODO start latest finish timer
        // TODO lincaizhao: check if workitemid is used for ytimer hashtable
        if (_latestStartTime != null) checkLatestFinishTimer(pmgr);
        if (pmgr != null) pmgr.updateObject(this);
    ...
    // lincaizhao: latest start time getter & setter
    public TimePoint getLatestStartTime() {
    	return _latestStartTime;
    }
    
    public void setLatestStartTime(TimePoint _start) {
    	_latestStartTime = _start;
    }

    public TimePoint getRegularStartTime() {
    	return _regularStartTime;
    }
    
    public void setRegularStartTime(TimePoint _start) {
    	_regularStartTime = _start;
    }
    
    public TimePoint getLatestFinishTime() {
    	return _latestStartTime.forward(_executeTime);
    }
    ...
    //limingyao
    public TimeSpan getExecuteTimeWithCaseLevel(){
    	double coe = _engine.getNetRunnerRepository().get(this).getCaseEmergencyLevel();
    	return _executeTime.multiply(coe);
    }
    
    public void setExecuteTime(TimeSpan _max) {
    	_executeTime = _max;
    }
    ...
    // limingyao: expiry flag getter & setter
    public boolean getIsEverExpired() {
    	return _isEverExpired;
    }
    public void setIsEverExpired(boolean iee){
    	_isEverExpired = iee;
    }
    ...
    public String toXML() {
        ...
        // TODO lincaizhao set temporal properties to xml string
        if (_executeTime != null) {
        	xml.append(StringUtil.wrap(String.valueOf(
        			_regularStartTime.getMillisecond()), "regularStartTime"));
        	xml.append(StringUtil.wrap(String.valueOf(
        			_latestStartTime.getMillisecond()), "latestStartTime"));
        	xml.append(StringUtil.wrap(String.valueOf(
        			_executeTime.getMillisecond()), "executeTime"));
        	
        	
        	//limingyao: set expiry flag to xml string
        	xml.append(StringUtil.wrap(String.valueOf(
        			_isEverExpired), "isEverExpired"));
        }
