...
import cn.edu.sysu.ss.workflow.temporal.*;
...
public class YNetRunner {
    ...
    //limingyao 
    private double _caseEmergencyLevel;
    private long _caseBufferTime;
    private double _factor; // used to modify workitem's execute time dynamically
    ...
    public YNetRunner(YPersistenceManager pmgr, YNet netPrototype, Element paramsData,
                      String caseID, TimeSpan validTime) throws YDataStateException, YPersistenceException {
         this();
        ...
        initialise(pmgr, netPrototype, _caseIDForNet, paramsData, new TimePoint(), validTime, 1.0) ;
    }

    //limingyao: Overload the contructor with the case emergencyLevel
    public YNetRunner(YPersistenceManager pmgr, YNet netPrototype, Element paramsData,
            String caseID, TimeSpan validTime, double emergencyLevel) throws YDataStateException, YPersistenceException {
		this();
		...
		initialise(pmgr, netPrototype, _caseIDForNet, paramsData, new TimePoint(), validTime, emergencyLevel) ;
    }
    ...
    public YNetRunner(YPersistenceManager pmgr, YNet netPrototype,
                      ...
                      Element incomingData, TimePoint start, TimePoint end)
            throws YDataStateException, YPersistenceException {

        this();
        
        //limingyao:Get the emergencyLevel of the root net
        String caseID = getRootCaseId();
        double emergencyLevel = 1.0;
        if(caseID != null && !caseID.isEmpty()){
        	emergencyLevel = _engine.getNetRunnerRepository().get(caseID).getCaseEmergencyLevel();
        }
        
        initialise(pmgr, netPrototype, caseIDForNet, incomingData, start, end.subtract(start), emergencyLevel) ;
    ...
    private void initialise(YPersistenceManager pmgr, YNet netPrototype,
                      YIdentifier caseIDForNet, Element incomingData, TimePoint start, TimeSpan validTime, double emergencyLevel)
            throws YDataStateException, YPersistenceException {
        ...
        _validTime = validTime.getMillisecond();
        
        //limingyao
        _caseEmergencyLevel = emergencyLevel;
        //time adjustment algorithm
        _caseBufferTime = _net.getSpecification().getCaseValidTime().subtract(_net.getSpecification().getCaseMaxTime()).getMillisecond();
        _factor = 1;
        
        prepare(pmgr);
        if (incomingData != null) _net.setIncomingData(pmgr, incomingData);
        initTimerStates();
    	// lincaizhao: start net runner
    	// _net.initTemporalStates(new TimeSpan(1000 * 60 * 24));
        YNetCalculator.initTemporalStates(getRootCaseId(), _net, start, start.forward(validTime), _caseEmergencyLevel);
    ...
    public long getValidTime() { return _validTime; }

    public void setValidTime(long time) { _validTime = time ; }

    
    //limingyao: case emergency level getter && setter
    public double getCaseEmergencyLevel() { return _caseEmergencyLevel; }
    
    public void setCaseEmergencyLevel(double level) { _caseEmergencyLevel = level; }
    
    public long getCaseBufferTime() { return _caseBufferTime; }
    
    public void setCaseBufferTime(long time) { _caseBufferTime = time; }
    
    public double getFactor() { return _factor; }
    
    public void setFactor(double f)  { _factor = f; }
    
    public String getRootCaseId(){
    	 String caseID = _caseIDForNet.get_idString();
         if (caseID.contains(".")){
         	String[] subStr = caseID.split("\\.");
         	String rootCaseID = "";
         	if(subStr.length > 0){
         		rootCaseID = subStr[0];
         	}
    ...
    private void cancelEnabledTask(YTask task, YPersistenceManager pmgr)
        ...
        if (wItem != null) {               //may already have been removed by task.cancel
            ...
            // a task with temporal properties won have other timer
            // lincaizhao: cancel latest start timer
            if (wItem.getLatestStartTime() != null) {
            	YTimer.getInstance().cancelTimerTask(wItem.getIDString());
            }
    ...
    private YWorkItem createEnabledWorkItem(YPersistenceManager pmgr,
        ...
        // set temporal properties to workitem
        // lincaizhao: set the latest start time to workitem
        TimeSpan executeTime = atomicTask.getMaxExecuteTime();
        if (executeTime != null) {
        	TimePoint regularStartTime = atomicTask.getRegularStartTime();
        	TimePoint latestStartTime = atomicTask.getLatestStartTime();
        	//TODO limingyao: modify the workitem execute time
        	workItem.setExecuteTime(executeTime.multiply(_caseEmergencyLevel));
        	workItem.setRegularStartTime(regularStartTime);
        	workItem.setLatestStartTime(latestStartTime);
        	// start latest start timer
        	// lincaizhao: check if workitem id is used in ytimer hashtable
        	workItem.checkLatestStartTimer(pmgr);
        	
        	// limingyao: set the expiry flag to workitem
        	boolean isEverExpired = workItem.getIsEverExpiredFromLog(pmgr);
        	workItem.setIsEverExpired(isEverExpired);
    ...
    private synchronized boolean completeTask(YPersistenceManager pmgr, YWorkItem workItem,
        ...
        if (taskExited) {
            ...
            //limingyao: time adjustment
            /*workItem.getTask().addExecuteTimeForCases(getRootCaseId(), ((new Date()).getTime()-workItem.get_firingTime().getTime()));
            adjustTime(workItem);
             */
   ...
   /** these two methods are here to support persistence of the IB Observer */
    ...
    //limingyao
    public void adjustTime(YWorkItem workitem){
    	long leftTime =  (new Date().getTime()) - workitem.getLatestFinishTime().getMillisecond();
    	if ( leftTime < 0){   //complete ahead of time
    		YNetCalculator.refreshTemporalStates(getRootCaseId(), _net, workitem.getTask(), leftTime);
    	}
    	else if( leftTime > 0 && _caseBufferTime >= leftTime){   //overtime
    		setCaseBufferTime(_caseBufferTime - leftTime);
    	}
    	else if( leftTime > 0 && _caseBufferTime < leftTime){
    		YNetCalculator.refreshTemporalStates(getRootCaseId(), _net, workitem.getTask(), _caseBufferTime-leftTime);
    		setCaseBufferTime(0);
