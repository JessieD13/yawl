...
import org.yawlfoundation.yawl.engine.time.YNetRunnerTimer;
...
public class YEngineRestorer {
    ...
    protected Set<YTimedObject> restoreTimedObjects() throws YPersistenceException {
        ...
        // TODO lincaizhao restore LatestStartTimer & LatestFinishTimer & NetRunnerTimer
        Set<YTimedObject> expiredLSTs = restoreLatestStartTimers();
        expiredObjects.addAll(expiredLSTs);
        Set<YTimedObject> expiredLFTs = restoreLatestFinishTimers();
        expiredObjects.addAll(expiredLFTs);
        Set<YTimedObject> expiredNetRunnerTimers = restoreNetRunnerTimers();
        expiredObjects.addAll(expiredNetRunnerTimers);
        
        return expiredObjects;
    }

    // lincaizhao: restore LatestStartTimers
    protected Set<YTimedObject> restoreLatestStartTimers() throws YPersistenceException {
    	_log.info("Restoring latest start timers - Starts");
        Set<YTimedObject> expiredTimers = new HashSet<YTimedObject>();
        Set<YLatestStartTimer> orphanedTimers = new HashSet<YLatestStartTimer>();
        Query query = _pmgr.createQuery("from YLatestStartTimer");
        for (Iterator it = query.iterate(); it.hasNext();) {
        	YLatestStartTimer lst = (YLatestStartTimer) it.next();
        	lst.setPersisting(true);
            
            // check to see if workitem still exists
            YWorkItem witem = _engine.getWorkItem(lst.getOwnerID()) ;
            if (witem == null)
                orphanedTimers.add(lst);
            else {
                long endTime = lst.getEndTime();

                // if the deadline has passed, time the workitem out
                if (endTime < System.currentTimeMillis())
                    expiredTimers.add(lst);
                else {
                    // reschedule the workitem's timer
                    YTimer.getInstance().schedule(lst, new Date(endTime));
                    // TODO lincaizhao: mark a state?
                    //witem.setTimerStarted(true);
                }
            }
        }

        for (YLatestStartTimer orphan : orphanedTimers) {
            _pmgr.deleteObject(orphan) ;                   // remove from persistence            
        }

        _log.info("Restoring latest start timers - Ends");
        return expiredTimers;
    }
    
 // lincaizhao: restore LatestStartTimers
    protected Set<YTimedObject> restoreLatestFinishTimers() throws YPersistenceException {
    	_log.info("Restoring latest finish timers - Starts");
        Set<YTimedObject> expiredTimers = new HashSet<YTimedObject>();
        Set<YLatestFinishTimer> orphanedTimers = new HashSet<YLatestFinishTimer>();
        Query query = _pmgr.createQuery("from YLatestFinishTimer");
        for (Iterator it = query.iterate(); it.hasNext();) {
        	YLatestFinishTimer lft = (YLatestFinishTimer) it.next();
        	lft.setPersisting(true);
            
            // check to see if workitem still exists
            YWorkItem witem = _engine.getWorkItem(lft.getOwnerID()) ;
            if (witem == null)
                orphanedTimers.add(lft);
            else {
                long endTime = lft.getEndTime();

                // if the deadline has passed, time the workitem out
                if (endTime < System.currentTimeMillis())
                    expiredTimers.add(lft);
                else {
                    // reschedule the workitem's timer
                    YTimer.getInstance().schedule(lft, new Date(endTime));
                    // TODO lincaizhao: mark a state?
                    //witem.setTimerStarted(true);
                }
            }
        ...
        for (YLatestFinishTimer orphan : orphanedTimers) {
            _pmgr.deleteObject(orphan) ;                   // remove from persistence            
        }

        _log.info("Restoring latest finish timers - Ends");
        return expiredTimers;
    }
    
    protected Set<YTimedObject> restoreNetRunnerTimers() throws YPersistenceException {
    	_log.info("Restoring net runner timers - Starts");
        Set<YTimedObject> expiredTimers = new HashSet<YTimedObject>();
        Set<YNetRunnerTimer> orphanedTimers = new HashSet<YNetRunnerTimer>();
        Query query = _pmgr.createQuery("from YNetRunnerTimer");
        for (Iterator it = query.iterate(); it.hasNext();) {
        	YNetRunnerTimer runnerTimers = (YNetRunnerTimer) it.next();
        	runnerTimers.setPersisting(true);
            /*
            // check to see if workitem still exists
            YWorkItem witem = _engine.getWorkItem(lft.getOwnerID()) ;
            if (witem == null)
        ...
        for (YNetRunnerTimer orphan : orphanedTimers) {
            _pmgr.deleteObject(orphan) ;                   // remove from persistence            
        }

        _log.info("Restoring net runner timers - Ends");
        return expiredTimers;
    }
