...
public class WorkItemRecord implements Cloneable, Comparable {
    ...
    // TODO lincaizhao temporal properties
    private String _regularStartTime;
    private String _latestStartTime;
    private String _executeTime;

    // limingyao: expiry flag
    private boolean _isEverExpired = false;
    ...
    //limingyao:overload the constructor
    public WorkItemRecord( Element e){
    	this();
    	fromXML(e);
    }
    ...
    // TODO lincaizhao temporal properties setter
    public void setRegularStartTime(String rstime) { _regularStartTime = rstime; }

    public void setLatestStartTime(String lstime) { _latestStartTime = lstime; }
    
    public void setExecuteTime(String time) { _executeTime = time; }

    // limingyao expiry flag setter
    public void setIsEverExpired(boolean iee) { _isEverExpired = iee; }
    ...
    // TODO lincaizhao temporal properties getter
    public String getRegularStartTime() { return _regularStartTime; }

    public String getLatestStartTime() { return _latestStartTime; }
    
    public String getExecuteTime() { return _executeTime; }
    
    //limingyao: Transform the rst Ms string to date string, for monitor service
    public String getRegularStartTimeAsDateString() {    	
    	if (getRegularStartTime() == "") return "";
    	
    	SimpleDateFormat dateFormatter = new SimpleDateFormat("yyyy-MM-dd H:mm:ss");    	
    	long rst = Long.parseLong(_regularStartTime);
        return dateFormatter.format(rst);
    }
    
    //limingyao: Transform the lst Ms string to date string, for monitor service
    public String getLatestStartTimeAsDateString() {    	
    	if (getLatestStartTime() == "") return "";
    	
    	SimpleDateFormat dateFormatter = new SimpleDateFormat("yyyy-MM-dd H:mm:ss");    	
    	long lst = Long.parseLong(_latestStartTime);
        return dateFormatter.format(lst);
    }
  
    // limingyao: expiry flag getter
    public boolean getIsEverExpired() { return _isEverExpired; }
    public boolean isEverExpired() { return _isEverExpired; }
    ...
    //limingyao 
    /**
     * compared by delay rate
     */
	public int compareTo(Object o) {
    	int flag; 
    	BigDecimal dr1 = new BigDecimal(this.getDelayRate());
    	BigDecimal dr2 = new BigDecimal(((WorkItemRecord)o).getDelayRate());
		flag = dr1.compareTo(dr2);
		
		return flag;
	}
    
    /**
     * the delay rate getter
     */
    public double getDelayRate(){
    	Date now = new Date();
		long nowTime = now.getTime();
		
		long latestStartTime = Long.parseLong(_latestStartTime);
		long regularStartTime = Long.parseLong(_regularStartTime);
		long bufferTime = latestStartTime - regularStartTime;
		
		if( nowTime <= regularStartTime) return 0;
		else if( nowTime >= latestStartTime ) return 100;
		else
			return (100.0*(double)(nowTime - regularStartTime) / (double)bufferTime);
    }

    /**
     *  check if the workitem is critical task instance, that is its rst == lst
     */
    public boolean isCritical(){
    	return ( _regularStartTime.equals(_latestStartTime));
    }
    
    /**
     * @return if the workitem is ahead of regularStartTime, return -1; 
     * else if it is behind the regular start time, return 1
     * else return 0
     */
    public int getProgressStatus(){
		...
		if( nowTime < regularStartTime )
			return -1;
		else if( nowTime > regularStartTime)
			return 1;
		else 
			return 0;
    }
    ...
    public String toXML() {
        ...
        xml.append(StringUtil.wrap(_allowsDynamicCreation, "allowsdynamiccreation"))
           ...
           // TODO lincaizhao add temporal properties to xml string
           .append(StringUtil.wrap(_latestStartTime, "latestStartTime"))
           .append(StringUtil.wrap(_regularStartTime, "regularStartTime"))
           .append(StringUtil.wrap(_executeTime, "executeTime"))
           
           //limingyao: add expiry flag to xml string
           .append(StringUtil.wrap(Boolean.toString(_isEverExpired), "isEverExpired"))
    ...
    //limingyao: xml to WorkItemRecord instance
    ...
    public void fromXML(Element instance) {
        if (instance != null) {
            //_id = Long.parseLong( instance.getChildText("id") );
            _specVersion = instance.getChildText("specversion");
            _specURI = instance.getChildText("specuri");
            _specIdentifier = instance.getChildText("specIdentifier");
            _caseID = instance.getChildText("caseid");
            _taskID = instance.getChildText("taskid");
            _taskName = instance.getChildText("taskname");
            _uniqueID = instance.getChildText("uniqueid");
            _enablementTime = instance.getChildText("enablementTime");
            _firingTime = instance.getChildText("firingTime");           
            _startTime = instance.getChildText("startTime");
            _completionTime = instance.getChildText("completionTime");
            
            _enablementTimeMs = instance.getChildText("enablementTimeMs");
            _firingTimeMs = instance.getChildText("firingTimeMs");
            _startTimeMs = instance.getChildText("startTimeMs");
            _completionTimeMs = instance.getChildText("completionTimeMs");
            _timerTrigger = instance.getChildText("timertrigger");
            _timerExpiry = instance.getChildText("timerexpiry");
            // lincaizhao
            _regularStartTime = instance.getChildText("regularStartTime");
            _latestStartTime = instance.getChildText("latestStartTime");
            _executeTime = instance.getChildText("executeTime");
            _status = instance.getChildText("status");
            _resourceStatus = instance.getChildText("resourceStatus");
            _startedBy = instance.getChildText("startedBy");
            _completedBy = instance.getChildText("completedBy");
            _customFormURL = instance.getChildText("customform");
            
            _isEverExpired = Boolean.parseBoolean( instance.getChildText("isEverExpired"));
        }
    }
