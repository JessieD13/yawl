...
public class InterfaceB_EngineBasedClient extends Interface_Client implements ObserverGateway {
    ...
    // TODO lincaizhao
    /**************** announce temporal timer events ****************/
    /**
     * 
     */
    public void announceLSTimerExpiry(YAnnouncement announcement) {
    	// TODO
    	YAWLServiceReference yawlService = announcement.getYawlService();
    	YWorkItem workItem = announcement.getItem();
    	_executor.execute(new Handler(yawlService, workItem, LS_TIMER_EXPIRED));
    	_logger.debug("announcing latest start timer expiry event");
    }
     ...
     * 
     */
    public void announceLFTimerExpiry(YAnnouncement announcement) {
    	// TODO
    	YAWLServiceReference yawlService = announcement.getYawlService();
    	YWorkItem workItem = announcement.getItem();
    	_executor.execute(new Handler(yawlService, workItem, LF_TIMER_EXPIRED));
    	_logger.debug("announcing latest finish timer expiry event");
    }
     ...
     * 
     */
    public void announceCaseTimeout(YAWLServiceReference yawlService, YIdentifier caseID) {
    	// TODO
    	_executor.execute(new Handler(yawlService, caseID, CASE_TIMEOUT));
    	_logger.debug("announcing case timeout event");
    }
