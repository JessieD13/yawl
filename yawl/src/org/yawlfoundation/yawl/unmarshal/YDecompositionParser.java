...
public class YDecompositionParser {
    ...
    private YTask parseTask(Element taskElem) {
        ...
        if ((task != null) && (! _version.isBetaVersion())) {
            ...
            parseTemporalParameter(task, taskElem);
        }
    ...
    // lincaizhao: parse temporal porperties
    private void parseTemporalParameter(YTask task, Element taskElem) {
    	Element crtTimeElem = taskElem.getChild("versionCreateTime", _yawlNS);
    	if (crtTimeElem != null) {
    		String crtTimeStr = crtTimeElem.getText();
    		TimePoint crtTimePoint;
    		if(!crtTimeStr.isEmpty()){
    			crtTimePoint = new TimePoint(Long.parseLong(crtTimeStr));
    			task.setVersionCreateTime(crtTimePoint);
    		}
    	}
    	
    	//limingyao
    	Element maxExeTimeElem = taskElem.getChild("maxExecuteTime", _yawlNS);
    	if (maxExeTimeElem != null) {
    		String exeTimeStr = maxExeTimeElem.getText();
    		TimeSpan exeTimeSpan = new TimeSpan(Long.parseLong(exeTimeStr));
    		task.setMaxExecuteTime(exeTimeSpan);
    	}
    	
    	Element minExeTimeElem = taskElem.getChild("minExecuteTime", _yawlNS);
    	if (minExeTimeElem != null) {
    		String exeTimeStr = minExeTimeElem.getText();
    		TimeSpan exeTimeSpan = new TimeSpan(Long.parseLong(exeTimeStr));
    		task.setMinExecuteTime(exeTimeSpan);
    	}
