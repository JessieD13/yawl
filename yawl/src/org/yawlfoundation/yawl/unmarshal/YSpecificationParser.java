...
class YSpecificationParser {
    ...
    private void parseSpecification(Element specificationElem, YSchemaVersion version)
        ...
        // lincaizhao: parse temporal properties
        String specCreTime = specificationElem.getChildText("specCreateTime", _yawlNS);
        Element specValidTimeElem = specificationElem.getChild("specValidTime", _yawlNS);
        String caseValidTime = specificationElem.getChildText("caseValidTime", _yawlNS);
        if (specCreTime != null) 
        	_specification.setSpecCreateTime(new TimePoint(Long.parseLong(specCreTime)));
        if (specValidTimeElem != null) {
            String specValidTimeStart = specValidTimeElem.getChildText("start", _yawlNS);
            String specValidTimeEnd = specValidTimeElem.getChildText("end", _yawlNS);
            _specification.setSpecValidTime(new TimeInterval(Long.parseLong(specValidTimeStart), Long.parseLong(specValidTimeEnd)));
        }
        if (caseValidTime != null)
        	_specification.setCaseValidTime(new TimeSpan(Long.parseLong(caseValidTime)));
        
        //Add by Hulin
        String caseMinTime = specificationElem.getChildText("caseMinTime" , _yawlNS );
        String caseMaxTime = specificationElem.getChildText("caseMaxTime" , _yawlNS );
       
        if (caseMinTime != null)
            _specification.setCaseMinTime( new TimeSpan(Long.parseLong(caseMinTime)));
        if (caseMaxTime != null)
            _specification.setCaseMaxTime( new TimeSpan(Long.parseLong(caseMaxTime)));
